...
export async function requestPasswordReset(data: {
  email: string;
}): Promise<{ success: boolean; error?: string; mockLink?: string }> {
  const { email } = data;
  ...
  try {
    ...
    if (error) {
      console.warn("[requestPasswordReset] Supabase reset email error:", error.message, "Status:", error.status);
      console.warn("[requestPasswordReset] Attempting to generate a local admin recovery link...");
      ...
      try {
        ...
        if (linkError) {
          console.error("[requestPasswordReset] Admin link generation error:", linkError.message);
        } else if (linkData?.properties?.action_link) {
          console.log("[requestPasswordReset] Generated bypass action link successfully.");
          return {
            success: true,
            mockLink: linkData.properties.action_link,
          };
        }
      } catch (adminErr) {
        console.error("[requestPasswordReset] Failed to generate admin fallback link:", adminErr);
      }
